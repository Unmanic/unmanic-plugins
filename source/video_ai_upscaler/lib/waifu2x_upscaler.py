...
class Waifu2xUpscaler:
    ...
    def _resolve_tool(self, scale):
        tool_bin = os.path.join(self.bin_dir, "waifu2x-ncnn-vulkan")
        tool_script = os.path.join(self.wrapper_dir, "waifu2x-upscale.sh")
        model_name = self.settings.get_setting("waifu2x_model")
        models_dir = get_latest_waifu2x_dir(self.share_dir)
        if not models_dir:
            models_dir = os.path.join(self.share_dir, "waifu2x-ncnn-vulkan")
        model_arg = os.path.join(models_dir, model_name)
        logger.debug(
            "Resolved Waifu2x tool: bin=%s script=%s model=%s scale=%s",
            tool_bin,
            tool_script,
            model_arg,
            scale,
    ...
    def run(self):
        def _runner(log_queue, prog_queue):
            ...
            noise_level = self.settings.get_setting("waifu2x_noise_level")
            ...
            scale = pick_waifu2x_scale(source_height, target_height)

            log_queue.put(
                "Waifu2x settings: model={}, noise_level={}, target_height={}, tile_size={}, gpu_id={}".format(
                    self.settings.get_setting("waifu2x_model"),
                    noise_level,
                    target_height,
                    tile_size,
            ...
            log_queue.put(
                "Waifu2x source: {}x{}, selected scale {}".format(
                    source_width,
                    source_height,
                    scale,
                )
            ...
            temp_dir = os.path.join(cache_dir, "waifu2x_tmp")
            ...
            try:
                if not success:
                    return
                log_queue.put(
                    "Waifu2x paths: cache_dir={}, frames_in={}, frames_out={}".format(
                        cache_dir,
                        frames_in_dir,
                        frames_out_dir,
                    )
                ...
                logger.exception("Waifu2x pipeline failed")
                success = False
            finally:
        ...
        if not success:
            logger.error("Waifu2x upscale failed.")
        return success
