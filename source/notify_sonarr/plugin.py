...
"""
# NOTE:
# This plugin is intentionally kept as close as possible to the notify_radarr plugin.
# If you add a feature here or fix a bug here, review notify_radarr and decide whether
...
ARR_NAME = "Sonarr"
ARR_NAME_LOWER = "sonarr"
ARR_API_CLASS = SonarrAPI
ARR_DEFAULT_HOST_URL = "http://localhost:8989"
ARR_ENTITY_LABEL = "series"
ARR_ENTITY_LABEL_TITLE = "Series"
ARR_ENTITY_LABEL_PLURAL = "series"
ARR_FILE_LABEL = "episode file"
ARR_FILE_LABEL_PLURAL = "episode files"
ARR_LIBRARY_EXAMPLE_PATH = "/tv/anime"
ARR_TAGS_VARIABLE_LABEL = "series"
ARR_QUALITY_EXAMPLE = "WEBDL-1080p"
ARR_ID_FIELD = "series_id"
ARR_TITLE_FIELD = "series_title"
ARR_EXTERNAL_ID_FIELD = "tvdb_id"
ARR_EXTERNAL_ID_LABEL = "TVDB"
ARR_QUEUE_LOOKUP_KEY = "title"
ARR_QUEUE_COMMAND_NAME = "DownloadedEpisodesScan"
ARR_REFRESH_COMMAND_NAME = "RescanSeries"
ARR_RENAME_COMMAND_NAME = "RenameFiles"
ARR_TRIGGER_REFRESH_LABEL = "Trigger series refresh on task complete"
ARR_IMPORT_LABEL = "Import episode on task complete"
ARR_TRIGGER_RENAME_LABEL = "Trigger Sonarr file renaming"
ARR_LOGGER_NAME = "Unmanic.Plugin.notify_sonarr"
ARR_WEBHOOK_TEST_MESSAGE = "Received Test webhook from Sonarr. Connection successful."
...
def get_arr_entity_from_processed_file(api, dest_path):
    # Sonarr can parse both the full path and the basename. Try the full path first
    # because it contains more context when identifying the series.
    basename = os.path.basename(dest_path)
    entity_data = api.get_parsed_title(dest_path)
    if not entity_data or not entity_data.get('series'):
        logger.debug("Failed to parse series info from full path '%s'. Trying with basename '%s'", dest_path, basename)
        entity_data = api.get_parsed_title(basename)

    series = entity_data.get('series', {}) if entity_data else {}
    return series.get('title'), series.get('id')


def queue_refresh(api, entity_id):
    return api.post_command(ARR_REFRESH_COMMAND_NAME, seriesId=entity_id)


def rename_entity_files(api, entity_id):
    # Sonarr renames specific episode file IDs rather than issuing a direct
    # rename command for the whole series.
    rename_list = api.get_episode_files_by_series_id(entity_id)
    file_ids = [episode['id'] for episode in rename_list]
    if not file_ids:
        return None
    return api.post_command(ARR_RENAME_COMMAND_NAME, seriesId=entity_id, files=file_ids)
...
def get_webhook_file_records(payload):
    if isinstance(payload.get('episodeFile'), dict):
        return [payload['episodeFile']]
    if isinstance(payload.get('episodeFiles'), list):
        return payload['episodeFiles']
    return []
...
def build_webhook_rule_base(entity, root_path, event_type):
    return {
        ...
        'series_title':    entity.get('title'),
        'series_id':       entity.get('id'),
...
def enrich_webhook_rule_base(api, flat_data_base):
    if not flat_data_base.get('series_id'):
        return
...
def update_mode(api, dest_path, rename_files):
    ...
    try:
        ...
        # Sonarr renames by first returning episode file IDs for the series, then issuing a
        # RenameFiles command with those IDs. If Sonarr returns no episode file IDs, there is
        # nothing to rename and the helper returns None instead of calling the rename command.
        if result is None:
            logger.warning("No %s found to rename for %s '%s' (ID: %s)",
                           ARR_FILE_LABEL_PLURAL, ARR_ENTITY_LABEL, entity_title, entity_id)
            return
