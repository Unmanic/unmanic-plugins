...
class Ac3Encoder(Encoder):
    ...
    def stream_args(self, stream_info, stream_id, encoder_name, filter_state=None):
        ...
        stream_args += [
            '-b:a', "{}k".format(self.settings.get_setting('ac3_average_bitrate')),
        ]
        if target_channels:
            stream_args += [
                '-ac:a:{}'.format(stream_id), str(target_channels),
    ...
    def get_ac3_average_bitrate_form_settings(self):
        values = {
            "label":          "Bitrate",
            "sub_setting":    True,
            "input_type":     "select",
